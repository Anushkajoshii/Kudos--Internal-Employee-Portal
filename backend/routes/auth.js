...
router.post('/login', (req, res) => {
  ...
  res.json({ token, user: { id: user.id, name: user.name, email: user.email, role: user.role, department: user.department, employee_id: user.employee_id } });
});
...
router.get('/me', authenticate, (req, res) => {
  const db = getDb();
  const user = db.prepare('SELECT id, employee_id, name, email, department, role, join_date, created_at FROM users WHERE id = ?').get(req.user.id);
